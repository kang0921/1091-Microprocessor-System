...
int main()
{
    unsigned int i, tmp, clock = 0, v1, v0, v_digit[4], result, result_digit[4] = {0};
    unsigned int index_7LED[8] = {Digit_1, Digit_2, Digit_3, Digit_4, Digit_5, Digit_6, Digit_7, Digit_8};
    unsigned int index_7LED_NUM[11] = {Number_0, Number_1, Number_2, Number_3, Number_4, Number_5, Number_6, Number_7, Number_8, Number_9, 0x00000};
    unsigned int cnt = 8;

    init();

    //LEDs
    tmp = (GPIO_PTC_PADIN >> 2) & 0xFF; // 讀取PTC ( Dip Switch ) 的輸入
    tmp = ~( (reverse( ~tmp ) << 8) | reverse( tmp ) );
    GPIO_PTB_GPIO = tmp;
    ...
    // set result_digit
    result_digit[0] = result / 1000;
    result_digit[1] = (result / 100) % 10;
    result_digit[2] = (result / 10) % 10;
    result_digit[3] = result % 10;

    if( result_zero == 0 ){
    	int flag = 0;
		for(i = 0; i<=3; i++){
			if(result_digit[i]==0 && flag == 0)
				result_digit[i] = 10;
			else
				flag = 1;
		}
    ...
    while(1)
    {
        ...
        for( i = 0; i<4; i++){
            //若V1及V0的十位數字為0則不顯示
            if( !(i == 0 && v_digit[i] == 0 ) && !( i == 2 && v_digit[i] == 0)){
                GPIO_PTA_GPIO = index_7LED[ 7 - i ];
                GPIO_PTD_GPIO = index_7LED_NUM[ v_digit[ i ] ];
                delay1(3500);
            }
        }
		...
		for(i=0; i<4; i++)
		{
			if( cnt - i < 5 )
			{
				...
				GPIO_PTD_GPIO = index_7LED_NUM[ result_digit[i] ];
				delay1(3500);
			}
		}

        if(clock > 10 && cnt > 4){
            cnt--;  // 控制跑馬的位置右移一格 (cnt 是為了控制跑馬的位置)
            clock = 0;  // clock是為了控制result要delay多久才跑進來
        }
        clock++;
    }
